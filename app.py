...
def create_hierchal_map(tree, highlight_path=None):
    ...
    fig, ax = plt.subplots(
        figsize=(max(7, widest_generation * 2), max(6, depth * 2.2))
    )
    ax.axis('equal')
    G = nx.Graph()
    parent_child_edges = []
    ...
    nx.draw_networkx_nodes(G, flipped_pos, ax=ax, node_color='lightblue', node_size=1200)
    nx.draw_networkx_labels(G, flipped_pos, labels=labels, ax=ax, font_size=10, font_weight='bold')
...
with left:
    ...
    tab_add, tab_parent, tab_spouse, tab_path, tab_delete = st.tabs(
        ["Add person", "Parent & child", "Spouse", "Find path", "Delete person"]
    )
    ...
    with tab_parent:
        st.markdown(
            """
            <div style="font-size:1.3rem; font-weight:700; line-height:1.5;
                        background-color:#1e2a38; color:#ffffff;
                        padding:0.9rem 1.1rem; border-radius:8px; margin-bottom:0.8rem;">
                Pick a child and connect one or both parents to them at once.<br>
                If both parents are known, connect both here in the same step.
            </div>
            """,
            unsafe_allow_html=True,
        )
        if len(tree) < 2:
            st.info("Add at least 2 people to connect them.")
        else:
            with st.form(key="add-parent-child-form"):
                ...
                parent_one = st.selectbox("Parent 1", options, format_func=get_label, key="pc_parent1")
                ...
                pc_submit = st.form_submit_button("Add relationship")
    ...
    with tab_spouse:
        if len(tree) < 2:
            ...
            with st.form(key="add-spouse-form"):
                ...
                spouse_two = st.selectbox("Spouse 2", options, format_func=get_label, key="sp_two")
    ...
    with tab_path:
        if len(tree) < 2:
            st.info("Add at least 2 people to find a path.")
        else:
            with st.form(key="find-path-form"):
                options = list(tree.keys())
                start_id = st.selectbox("From", options, format_func=get_label, key="path_start")
                end_id = st.selectbox("To", options, format_func=get_label, key="path_end")
                path_submit = st.form_submit_button("Find path")
            if path_submit:
                if start_id == end_id:
                    st.info(f"{get_name(start_id)} is the same person — distance 0.")
                    st.session_state.highlight_path = None
                else:
                    neighbor_dict = bfs(tree, start_id, 'all')
                    if end_id not in neighbor_dict:
                        st.error(f"No relationship path between {get_name(start_id)} and {get_name(end_id)}.")
                        st.session_state.highlight_path = None
                    else:
            ...
            if st.session_state.highlight_path:
                if st.button("Clear highlighted path"):
                    st.session_state.highlight_path = None
                    st.rerun()
...
with right:
    ...
    if st.session_state.tree:
        ...
        st.info("Add people and relationships to see the tree here.")